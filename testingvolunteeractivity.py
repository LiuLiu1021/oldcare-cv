...
用法：
python /home/reed/Desktop/code/oldcare/testingvolunteeractivity.py
python /home/reed/Desktop/code/oldcare/testingvolunteeractivity.py --filename /home/reed/Desktop/code/oldcare/tests/desk_01.mp4
'''
...
model_path = '/home/reed/Desktop/code/oldcare/models/face_hog.pickle'
...
while True:
    ...
    if not input_video:
        frame = cv2.flip(frame, 1)# 读取一帧

    frame = imutils.resize(frame,width=VIDEO_WIDTH,height=VIDEO_HEIGHT)  # 压缩，加快识别速度

    face_location_list, names = faceutil.get_face_location_and_name(frame)# 定位一帧中的所有人脸信息和人脸位置
    ...
    for ((left, top, right, bottom), name) in zip(face_location_list,names):  # 处理这一帧的每个人
        ...
        if person_type == 'volunteer':  # 如果检测到有义工存在
            # 获得义工位置
            volunteer_face_center = (int((right + left) / 2),int((top + bottom) / 2))#中心点
    ...
    for i in volunteer_centroids:
        for j_index, j in enumerate(old_people_centroids):
            pixel_distance = dist.euclidean(i, j)# 欧几里得距离
