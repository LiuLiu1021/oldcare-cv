...
用法：
python /home/reed/Desktop/code/oldcare/collectingfaces.py --id 166 --imagedir /home/reed/Desktop/code/oldcare/images/capturefaces
...
# 全局参数
audio_dir = '/home/reed/Desktop/code/oldcare/audios'# 虚拟机上的存储音频的路径

imagedir='/home/reed/Desktop/code/oldcare/images/faces'

# # 传入参数
# ap = argparse.ArgumentParser()
# # 为了在命令行中避免位置参数的bug（容易忘了顺序），可以使用可选参数，这个有点像关键词传参，但是需要在关键词前面加 --
# ap.add_argument("-ic", "--id", required=True, help="")
# ap.add_argument("-id", "--imagedir", required=True, help="")
# args = vars(ap.parse_args())
def collectingfaces(id,idcard):
    # 控制参数
    error = 0
    start_time = None
    limit_time = 2  # 2 秒
    ...
    while True:
        ...
        if counter <= 10:  # 放弃前10帧
            continue
        image = cv2.flip(image, 1)# 图像翻转，1 水平翻转,0 垂直反转, -1 水平垂直翻转

        if error == 1:# 没有检测到人脸
            end_time = time.time()
            difference = end_time - start_time
            print(difference)
            if difference >= limit_time:# 如果规定时间内都没有检测到人脸，就再来一次
                error = 0
    ...
    # 新建目录
    if os.path.exists(os.path.join(imagedir, str(id))):
        shutil.rmtree(os.path.join(imagedir, str(id)), True)
    os.mkdir(os.path.join(imagedir, str(id)))

    path = '/home/reed/Desktop/code/oldcare/faces/' + idcard + '.jpg'
    grab, frame = cam.read()
    ...
    for action in action_list:
        ...
        for i in range(15):
            ...
            image_name = os.path.join(imagedir, str(id),action + '_' + str(counter) + '.jpg')
